...
class Attention_reorder(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('sorted_indices_asc: ', sorted_indices_asc[0, 0, :])
...
class Block_reorder(nn.Module):
    def __init__(self, 
        ...
        self.norm1 = norm_layer(dim)
        if counter in transformer_blocks:
            if counter == (self.depth//2 + 1 if self.depth %2 !=0 else self.depth //2):
                self.mixer = Attention_reorder(
                dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
                qk_norm=qk_scale,
...
class MambaVisionLayer_reorder(nn.Module):
    ...
    def __init__(self,
        ...
        if conv:
            ...
            self.blocks = nn.ModuleList([Block_reorder(dim=dim,
                                               counter=i, 
                                               transformer_blocks=transformer_blocks,
                                               num_heads=num_heads,
                                               mlp_ratio=mlp_ratio,
