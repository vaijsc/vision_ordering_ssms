...
class Cluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24,
        ...
        super().__init__()
        self.heads = heads # 4 
        self.head_dim = head_dim # 24
        self.f = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for similarity 32 -> 4 * 24
        self.proj = nn.Conv2d(heads * head_dim, out_dim, kernel_size=1)  # for projecting channel number 4 * 24 -> 32
        self.v = nn.Conv2d(dim, heads * head_dim, kernel_size=1)  # for value 32 -> 4 * 24
        self.sim_alpha = nn.Parameter(torch.ones(1))
        self.sim_beta = nn.Parameter(torch.zeros(1))
        self.centers_proposal = nn.AdaptiveAvgPool2d((proposal_w, proposal_h)) # AdaptiveAvgPool2d(output_size=(2, 2))
        self.fold_w = fold_w # 8
        self.fold_h = fold_h # 8
        self.return_center = return_center # False
        self.ord_token = nn.Parameter(torch.randn(proposal_w * proposal_h))

    def forward(self, x):  # [b,c,w,h]
        # import ipdb; ipdb.set_trace()
        # x torch.Size([128, 32, 56, 56])
        # torch.Size([128, 196, 320]) -> [128, 320, 14, 14]
        import numpy as np
        length = np.sqrt(int(x.size(1)))
        length = int(length)
        x = x.view(x.size(0), x.size(2), length, length)
        value = self.v(x) # torch.Size([128, 96, 14, 14])
        x = self.f(x) # torch.Size([128, 96, 14, 14])
        x = rearrange(x, "b (e c) w h -> (b e) c w h", e=self.heads) # self.heads=4, torch.Size([512, 24, 14, 14])
        value = rearrange(value, "b (e c) w h -> (b e) c w h", e=self.heads) # torch.Size([512, 24, 14, 14])
        if x.size(2) == 7:
            self.fold_h = 1
            self.fold_w = 1
        # fold_w = 2, fold_h = 2
        if self.fold_w > 1 and self.fold_h > 1:
            ...
            # x torch.Size([2048, 24, 7, 7])
            value = rearrange(value, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.fold_w, f2=self.fold_h) # torch.Size([512, 24, 7, 7])
            # value torch.Size([2048, 24, 7, 7])
            
        b, c, w, h = x.shape # value torch.Size([2048, 24, 7, 7])
        # import ipdb; ipdb.set_trace()
        
        centers = self.centers_proposal(x)  # [b,c,C_W,C_H], we set M = C_W*C_H and N = w*h 
        # centers torch.Size([2048, 24, 2, 2])
        
        value_centers = rearrange(self.centers_proposal(value), 'b c w h -> b (w h) c')  # [b,C_W,C_H,c]
        # value_centers torch.Size([2048, 4, 24])
        
        b, c, ww, hh = centers.shape

        # centers.reshape(b, c, -1).permute(0, 2, 1) ---- torch.Size([32768, 4, 24])
        # x.reshape(b, c, -1).permute(0, 2, 1) ---- torch.Size([32768, 49, 24])
        ...
        sim = sim * mask # torch.Size([32768, 4, 49])
        value2 = rearrange(value, 'b c w h -> b (w h) c')  # [B,N,D] --> torch.Size([2048, 49, 24])
        ...
        if self.return_center:
            ...
            # out torch.Size([2048, 49, 24])
            out = rearrange(out, "b (w h) c -> b c w h", w=w)
            # out torch.Size([2048, 24, 7, 7])
        ...
        out = rearrange(out, "(b e) c w h -> b (e c) w h", e=self.heads) # torch.Size([128, 96, 14, 14])
        out = self.proj(out) # out torch.Size([128, 320, 14, 14])
...
class Block(nn.Module):
    def __init__(self, 
        ...
        if counter in transformer_blocks:
            self.mixer = Cluster(dim, out_dim = dim)
        else:
