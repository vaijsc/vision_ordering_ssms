...
DR=0.2
MESA=0.25
RUN_FILE="/home/ubuntu/workspace/mambavision_1/mambavision/train_1.py"
torchrun --master-port=21381 --nproc_per_node=4 $RUN_FILE --mesa ${MESA} --input-size 3 224 224 --crop-pct=0.875 \
 --data_dir=$DATA_PATH --model $MODEL --amp --weight-decay ${WD} --batch-size $BS --tag $EXP --lr $LR --warmup-lr $WR_LR \
> >(tee -a /home/ubuntu/workspace/mambavision_1/mambavision/result/mambaV_test4.txt) 2>&1
...
# use just 2 keys for two block in each initial stage
# edit full key for block + double check mamba