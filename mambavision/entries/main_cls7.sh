#!/bin/bash
#SBATCH --job-name=mv_cls7
#SBATCH --error=/lustre/scratch/client/vinai/users/ducna22/workspace/mambavision_1/mambavision/result/mambaV_cls7_1.txt
#SBATCH --nodes=1
#SBATCH --gpus-per-node=1
#SBATCH --nodelist=sdc2-hpc-dgx-a100-012
...
# DR=0.38
MESA=0.25
RUN_FILE="/lustre/scratch/client/vinai/users/ducna22/workspace/mambavision_1/mambavision/train_cls7.py"
torchrun --master_port 12361 --nproc_per_node=1 $RUN_FILE --mesa ${MESA} --input-size 3 224 224 --crop-pct=0.875 \
--data_dir=$DATA_PATH --model $MODEL --amp --weight-decay ${WD} --batch-size $BS --tag $EXP --lr $LR --warmup-lr $WR_LR > '/lustre/scratch/client/vinai/users/ducna22/workspace/mambavision_1/mambavision/result/log_cls.txt' 2>&1
