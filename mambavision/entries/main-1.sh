...
RUN_FILE="/root/workspace/mambavision_1/mambavision/train.py"
...
torchrun --master-port=12366 --nproc_per_node=2 $RUN_FILE --mesa ${MESA} --input-size 3 224 224 --crop-pct=0.875 \
--data_dir=$DATA_PATH --model $MODEL --amp --weight-decay ${WD} --batch-size $BS --tag $EXP --lr $LR --warmup-lr $WR_LR \
> '/root/workspace/mambavision_1/mambavision/result/log.txt' 2>&1
