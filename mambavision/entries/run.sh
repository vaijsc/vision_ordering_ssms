...
torchrun --master-port=21382 --nproc_per_node=1 $RUN_FILE --mesa ${MESA} --input-size 3 224 224 --crop-pct=0.875 \
 --data_dir=$DATA_PATH --model $MODEL --amp --weight-decay ${WD} --batch-size $BS --tag $EXP --lr $LR --warmup-lr $WR_LR # \
# > >(tee -a /home/ubuntu/workspace/mambavision_1/mambavision/result/mambaV_test1.txt) 2>&1
